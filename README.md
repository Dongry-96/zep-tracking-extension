...
이 문서는 대표 매니저가 중앙 세팅을 만들고, 다른 매니저들이 같은 결과를 함께 조회할 수 있도록 처음부터 끝까지 설명합니다.
...
-> Apps Script
-> Google Sheet 저장
-> 각 매니저의 Chrome Extension에서 조회
```

중요한 운영 원칙은 아래와 같습니다.

- **ZEP API 연결은 트랙당 1개**만 사용합니다.
- 같은 트랙에서는 대표 매니저 1명이 `Cloudflare Worker`, `Apps Script`, `Google Sheet`를 구성합니다.
...
| 역할 | 해야 할 일 |
| --- | --- |
| 대표 매니저 | Google Sheet 생성, Apps Script 배포, Cloudflare Worker 배포, ZEP API 연결 |
| 다른 매니저 | GitHub에서 코드 받기, Chrome 확장 프로그램 설치, 공유받은 Worker URL과 Google Sheet URL 입력 |

## 대표 매니저 1단계: Google Sheet 만들기
...
## 대표 매니저 4단계: Cloudflare Worker 설정하기
...
## 모든 매니저 6단계: Chrome 확장 프로그램 설치하기
